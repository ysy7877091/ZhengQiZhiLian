...
public class ProblemSumManagerJuRunnable implements PublicMethodInterface {
    private PublicInterface DataListener;
    private List<PublicBeen> list =new ArrayList<>();
    private String id;
    public ProblemSumManagerJuRunnable(String id){
        this.id=id;
    }
    @Override
        ...
        new Thread(){
            ...
            public void run() {
                ...
                try{
                    ...
                    // 调用的方法名称
                    String methodName = "Check_Problem";
                    // EndPoint
                    String endPoint = Path.get_faGai_Url();
                    // SOAP Action
                    String soapAction = "http://tempuri.org/Check_Problem";
                    // 指定WebService的命名空间和调用的方法名
                    SoapObject rpc = new SoapObject(nameSpace, methodName);
                    rpc.addProperty("gljID",id);
                    ...
                    if(count1>0) {
                        ...
                        for (int i = 0; i < count1; i++) {
                            ...
                            Log.e("warn",soapProvince.getProperty("LoginUserList").toString());
                            pb.setProject_state(soapProvince.getProperty("State").toString());
                            pb.setProject_id(soapProvince.getProperty("ID").toString());
                            pb.setPROBLEMCONTENT(soapProvince.getProperty("PROBLEMCONTENT").toString());
                            pb.setIMAGESOURCE(soapProvince.getProperty("IMAGESOURCE").toString());
                            pb.setProject_TIME(soapProvince.getProperty("TIME").toString());
                            pb.setProject_LoginUserList(soapProvince.getProperty("LoginUserList").toString());
                            list.add(pb);
                        }
